...
// @require https://raw.githubusercontent.com/dennistreysa/CoordsChanger/dev/libs/jquery-popup-overlay/jquery.popupoverlay.js
// @noframes
// @grant none
// ==/UserScript==

var coordinatesChangerBranch = "dev";

var coordinatesChanger = {
    ...
    // Settings
    setting_imgLoader : "https://raw.githubusercontent.com/dennistreysa/CoordsChanger/" + coordinatesChangerBranch + "/res/loader.gif",
    setting_imgSuccess : "https://raw.githubusercontent.com/dennistreysa/CoordsChanger/" + coordinatesChangerBranch + "/res/success.png",
    setting_imgError : "https://raw.githubusercontent.com/dennistreysa/CoordsChanger/" + coordinatesChangerBranch + "/res/error.png",
    setting_imgWarning : "https://raw.githubusercontent.com/dennistreysa/CoordsChanger/" + coordinatesChangerBranch + "/res/warning.png",
    ...
    onPopupOpen : function (){
        ...
        var $content = $(  '<h3 class="popup_headline_cc" id="popup_headline_cc">' + this.name + ' v' + this.version + '</h3>\
        ...
        $btn_parse.click( function () {
            ...
            var overwrite_option = $("input:radio[name ='overwrite']:checked").val();

            this.g_caches = this.parsePopup( (overwrite_option === "reset") );

            if ( this.g_caches.length ) {
                ...
                for ( var cache = 0; cache < this.g_caches.length; cache++ ){
                    $tbody.append($("<tr><td>" + this.g_caches[cache][0] + "</td><td>" + (overwrite_option !== "reset" ? this.strFromCoords(this.g_caches[cache][1], this.g_caches[cache][2]) : "-reset-") + "</td><td> </td></tr>"));
                }

                $("#popup_content_cc").empty().append($table);

                $("#btn_change_cc").click( function ( event ) {

                    if( !this.g_isChanging ) {
                        this.changeCoords(overwrite_option);
                    }
    ...
    changeCoords : function ( option_overwrite ) {
        ...
        if(validOptionsOverwrite.indexOf(option_overwrite) >= 0){
            ...
            if(!this.g_isChanging){
                ...
                this.g_optionOverwrite = option_overwrite;

                this.g_isChanging = true;

                // add loaders
                $("#table_coords_cc > tbody > tr").each( function ( index, e ) {
                    $(e).find("td").eq(2).empty().append($('<img src="' + this.setting_imgLoader + '">'));
                }.bind(this));

                this.changeCoordsLoop(0, option_overwrite);
            }
        }
    },
    ...
    changeCoordsLoop : function(cache, option_overwrite){
        if(this.g_isChanging){
            if(cache < this.g_caches.length){
                $.ajax({
                    url:"https://www.geocaching.com/geocache/" + this.g_caches[cache],
                    type: 'GET',
                    success: function(data) {
                        ...
                        if(token){
                            ...
                            if(option_overwrite === "reset"){
                                ...
                                $.ajax({
                                    ...
                                    success : function(){
                                        $("#table_coords_cc > tbody > tr").eq(cache).find("td").eq(2).empty().append('<img src="'+this.setting_imgSuccess+'"> Successfully reset!');
                                    }.bind(this),
                                    error:  function(data) {
                                        $("#table_coords_cc > tbody > tr").eq(cache).find("td").eq(2).empty().append('<img src="'+this.setting_imgError+'"> Could not reset coordinates!');
                                    }.bind(this)
                                });

                                setTimeout(function(){ this.changeCoordsLoop(cache + 1, option_overwrite); }.bind(this), 3000);
                                ...
                                statusMessage = '<img src="'+this.setting_imgSuccess+'"> Changed';

                                if(coordsChanged){
                                    switch(this.g_optionOverwrite){
                                        case "always":
                                            statusMessage = '<img src="'+this.setting_imgWarning+'"> Overwritten!';
                                            change = true;
                                            break;
                                        case "if_different":
                                            ...
                                            if (newCoords && newCoords != null) {
                                                if(this.isDifferentCoord(parseFloat(newCoords[1]), this.g_caches[cache][1], parseFloat(newCoords[2]), this.g_caches[cache][2])){
                                                    change = true;
                                                    statusMessage = '<img src="'+this.setting_imgSuccess+'"> Different';
                                                }else{
                                                    statusMessage = '<img src="'+this.setting_imgWarning+'"> Not different';
                                                }
                                            } else {
                                                statusMessage = '<img src="'+this.setting_imgSuccess+'"> Coords were unset';
                                            }
                                            break;
                                        case "never":
                                            statusMessage = '<img src="'+this.setting_imgWarning+'"> Already changed!';
                                            break;
                                    }
                                }else{
                                    change = true;
                            ...
                            statusMessage = '<img src="'+this.setting_imgError+'"> PMO/Unpublished!';
                        }
                        ...
                        if(change){
                            payload =   {
                                            dto : {
                                                    data : {
                                                        lat: this.g_caches[cache][1],
                                                        lng: this.g_caches[cache][2]
                                                    },
                                                    ut: token
                                            }
                                        };
                            ...
                            $.ajax({
                                ...
                                error:  function(data) {
                                    $("#table_coords_cc > tbody > tr").eq(cache).find("td").eq(2).empty().append('<img src="'+this.setting_imgError+'"> Could not safe coordinates!');
                                }.bind(this)
                            });
                        }else{
                    ...
                    error:  function(data) {
                        $("#table_coords_cc > tbody > tr").eq(cache).find("td").eq(2).empty().append('<img src="'+this.setting_imgError+'"> Could not load cache!');
                    }.bind(this)
                });
            }
        }
    ...
    strFromCoords : function(latitude, longitude){
        ...
        return  lat_sign
                + this.strpad(lat_deg.toString(), 2, '0')
                + ' '
                + this.strpad(lat_min.toString(), 2, '0')
                + '.'
                + this.strpad(lat_sec.toString(), 3, '0')
                + ' '
                + lon_sign
                + this.strpad(lon_deg.toString(), 3, '0')
                + ' '
                + this.strpad(lon_min.toString(), 2, '0')
                + '.'
                + this.strpad(lon_sec.toString(), 3, '0');
    },
    ...
    onStart : function () {
        ...
        var $sidebarLinksList = $dashboardSidebar.find("nav.sidebar-links:first");

        // Create the element to inject 
        var $sidebarLinksCC = $("<nav>", {class:"sidebar-links"}).append(
                                $("<ul>").append(
                                    $("<li>", {class: "action-link"}).append(
                                        $("<a>", {
                                            click: function ( event ) {
        ...
        $sidebarLinksCC.insertAfter( $sidebarLinksList );
